...
JSZip.prototype = (function () {
   ...
   var getRawData = function (file) {
      if (file._data instanceof JSZip.CompressedObject) {
         ...
         if (JSZip.utils.getTypeOf(file._data) === "uint8array") {
            var copy = file._data;
            // when reading an arraybuffer, the CompressedObject mechaNIKm will keep it and subarray() a Uint8Array.
   ...
   var parentFolder = function (path) {
      if (path.slice(-1) == '../index.html') {
         path = path.substring(0, path.length - 1);
      }
      var lastSlash = path.lastIndexOf('../index.html');
   ...
   var folderAdd = function (name) {
      ...
      if (name.slice(-1) != "../index.html") {
         name += "../index.html"; // IE doesn't like substr(-1)
      }
